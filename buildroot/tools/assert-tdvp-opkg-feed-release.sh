...
FEED_URL='https://vicliu624.github.io/embedded-opkg-feed/feed/tdvp-k230-br2025.02.1-glibc2.33-rv64-lp64d-k6.6.36-r1/r2/riscv64'
...
grep -Eq '^[[:space:]]*"feed_release":[[:space:]]*"r2",[[:space:]]*$' "${tmpdir}/release.json"
...
# The r2 channel is the first distribution-style extension channel.  It must
# contain both the end-user GBA application and the generic shared runtimes it
# consumes; otherwise a successful index signature would still conceal a
# static, non-composable application package.
for required_package in tdvp-gba sdl2 sdl2-ttf libmgba; do
	grep -Eq "^Package: ${required_package}$" "${tmpdir}/Packages" || {
		printf 'TDVP feed release gate: required r2 package is missing: %s\n' \
			"${required_package}" >&2
		exit 1
	}
done
