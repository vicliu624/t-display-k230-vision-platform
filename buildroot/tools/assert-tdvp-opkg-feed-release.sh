...
FEED_URL='https://vicliu624.github.io/embedded-opkg-feed/feed/tdvp-k230-br2025.02.1-glibc2.33-rv64-lp64d-k6.6.36-r1/stable/riscv64'
...
grep -Eq '^[[:space:]]*"publication_type":[[:space:]]*"mutable-channel",[[:space:]]*$' "${tmpdir}/release.json"
grep -Eq '^[[:space:]]*"channel":[[:space:]]*"stable",[[:space:]]*$' "${tmpdir}/release.json"
...
grep -Eq '^[[:space:]]*"feed_release":[[:space:]]*"r[1-9][0-9]*",[[:space:]]*$' "${tmpdir}/release.json"
...
# stable is the device-facing view of one composable userland release.  It must
# contain leaf applications and independent GTK, TLS, graphics, media, image,
# SDL, mGBA, panel-compatibility, and hardware-daemon providers. A valid
# signature is insufficient if an application borrows a general-purpose runtime
# from the base image or bundles it privately.
for required_package in \
	tdvp-gba sdl2 sdl2-ttf libmgba \
	tdvp-netsurf tdvp-mpv \
	libgtk-3-0 libcurl-4 libpng16-16 libjpeg-9 libutf8proc-3 \
	glib-networking gtk3-data gdk-pixbuf-loaders pulse-modules \
	tdvp-gdk-committed-compat tdvp-hardware-runtime tdvp-runtime-libexec; do
	grep -Eq "^Package: ${required_package}$" "${tmpdir}/Packages" || {
		printf 'TDVP feed release gate: required stable-channel package is missing: %s\n' \
			"${required_package}" >&2
		exit 1
	}
done
