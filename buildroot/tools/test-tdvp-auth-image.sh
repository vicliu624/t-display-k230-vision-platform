...
printf 'inert windowed locker fixture\n' > "$root/usr/bin/gtklock"
printf 'tdvp:!:1:0:99999:7:::\n' > "$root/etc/shadow"
sed 's/\r$//' "$project/buildroot/k230-sdk-overlay/package/swaylock/src/swaylock" > "$root/etc/pam.d/swaylock"
sed 's/\r$//' "$project/buildroot/k230-sdk-overlay/package/gtklock/src/gtklock" > "$root/etc/pam.d/gtklock"
...
for path in /usr/sbin/unix_chkpwd /usr/bin/swaylock /usr/bin/gtklock /etc/shadow /etc/pam.d/swaylock /etc/pam.d/gtklock /etc/greetd/config.toml; do
...
debugfs -w -R 'set_inode_field /usr/bin/gtklock mode 0100755' "$test_dir/good.ext4" >/dev/null 2>&1
...
debugfs -w -R 'set_inode_field /etc/pam.d/gtklock mode 0100644' "$test_dir/good.ext4" >/dev/null 2>&1
...
reject_metadata 'set_inode_field /usr/bin/gtklock mode 0104755'
reject_metadata 'rm /usr/bin/gtklock'
reject_metadata 'set_inode_field /etc/shadow mode 0100644'
reject_metadata 'rm /usr/sbin/unix_chkpwd'
for failure in pam gtklock-pam greeter; do
    cp "$test_dir/good.ext4" "$test_dir/bad.ext4"
    if [[ "$failure" == pam || "$failure" == gtklock-pam ]]; then
        locker=swaylock
        [[ "$failure" != gtklock-pam ]] || locker=gtklock
        destination=/etc/pam.d/$locker
        printf 'auth sufficient pam_permit.so\n' > "$test_dir/replacement"
        sed 's/\r$//' "$root/etc/pam.d/$locker" >> "$test_dir/replacement"
    else
    ...
    if [[ "$failure" == pam || "$failure" == gtklock-pam ]]; then
        grep -Fq "unexpected $locker PAM policy" "$test_dir/log"
    else
        grep -Fq 'expected authenticated greeter default' "$test_dir/log"
    fi
done
echo 'TDVP auth image: PASS production permission rule, real ext4 metadata and 13 rejected regressions'
