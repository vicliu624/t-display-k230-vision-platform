...
{
	printf("usage: %s [--device /dev/dri/card0] [--seconds N]\n", argv0);
	printf("\n");
	printf("Draws color bars through DRM/KMS for board bring-up validation.\n");
}
...
static void tdvp_fill_color_bars(const struct tdvp_display *display,
				 const struct tdvp_buffer *buffer)
{
	const struct tdvp_color colors[] = {
		{255, 0, 0, 255},
		{0, 255, 0, 255},
		{0, 0, 255, 255},
		{255, 255, 255, 255},
	...
	for (y = 0; y < display->mode.vdisplay; ++y) {
		...
		for (x = 0; x < display->mode.hdisplay; ++x) {
			unsigned int bar =
				(x * (sizeof(colors) / sizeof(colors[0]))) /
			      ...
			      const struct tdvp_buffer *buffer)
{
	...
	ret = 0;
	if (tdvp_add_prop(req, display->conn_id, &display->conn_props, "CRTC_ID",
			  display->crtc_id) < 0)
		ret = -1;
	if (tdvp_add_prop(req, display->crtc_id, &display->crtc_props, "MODE_ID",
			  display->mode_blob_id) < 0)
		ret = -1;
	if (tdvp_add_prop(req, display->crtc_id, &display->crtc_props, "ACTIVE",
			  1) < 0)
		ret = -1;
...
static int tdvp_run_smoke(struct tdvp_display *display, unsigned int seconds)
{
	struct tdvp_buffer buffer;
	char name[5];
	unsigned int i;
	...
	for (i = 0; i < sizeof(tdvp_format_candidates) /
		...
		tdvp_fill_color_bars(display, &buffer);

		if (tdvp_commit_buffer(display, &buffer) == 0) {
			printf("tdvp-display-smoke: PASS plane=%u format=%s\n",
			       display->plane_id, name);
			printf("tdvp-display-smoke: screen should show red/green/blue/white bars for %u seconds\n",
			       seconds);
			tdvp_sleep_seconds(seconds);
			tdvp_destroy_buffer(display, &buffer);
			return 0;
		}
	...
	fprintf(stderr,
		"tdvp-display-smoke: no candidate format could be presented\n");
	return -1;
}
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; ++i) {
		if (strcmp(argv[i], "--device") == 0 && i + 1 < argc) {
			...
			seconds = (unsigned int)strtoul(argv[++i], NULL, 10);
	...
	if (tdvp_open_display(&display, device) < 0) {
		tdvp_close_display(&display);
		return 1;
	}

	ret = tdvp_run_smoke(&display, seconds);
	tdvp_close_display(&display);
