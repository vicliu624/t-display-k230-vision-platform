...
if git -C "${CPU1_CHECKOUT_DIR}" sparse-checkout -h >/dev/null 2>&1; then
	...
	git -C "${CPU1_CHECKOUT_DIR}" sparse-checkout set --no-cone \
		...
		canmv_k230/boards/Kconfig canmv_k230/tools \
...
make k230_canmv_v3p0_defconfig
for setting in \
...
make -C "${SDK_RTSMART_SRC_DIR}" kernel CROSS_COMPILE="${CPU1_CROSS_COMPILE}"
make -C "${SDK_OPENSBI_SRC_DIR}" all CROSS_COMPILE="${CPU1_CROSS_COMPILE}"
