...
模型仅能编译不能作为发布依据。当前视觉候选 MMZ 为 128 MiB，并不是全部可用的语音模型容量；摄像头缓冲区也消耗该预算。旧的 500 MiB ASR 上限已不适用，具体模型入选前必须重新计算内存布局，并成对更新 Linux DT、CPU1 固件和镜像校验。
