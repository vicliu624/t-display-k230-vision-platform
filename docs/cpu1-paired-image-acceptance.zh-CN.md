...
本文保存 a57e99c 及 2026-09-08 初期候选的构建和测试过程。当前镜像获取方式见
[快速开始](getting_started.zh-CN.md)，后续 AI 结果见
[2026-09-09 验证记录](cpu1-ai-status-remote-validation-20260909.zh-CN.md)。
下面的旧镜像哈希和故障状态只适用于各自记录，不应作为新卡的下载或验收基线。

2026-09-08 更新：后续 CI `34149081510` 镜像在新卡上确认 CPU1 停在 I2C4
时钟阶段。已定位并回归修复 MMIO 解除映射缺陷，但修复固件尚未通过冷启动；
详见 [MMIO 缺陷与测试边界](cpu1-mmio-unmap-validation.zh-CN.md)。以下历史构建
记录不能视为当前摄像头、AI 或蓝牙功能验收通过。
...
保留该记录用于追溯启动缺陷，新卡测试应使用包含后续修复的配对候选。

## 首次启动

1. 保留原卡，使用备用 microSD。核对烧录工具选择的物理磁盘，避免覆盖电脑磁盘。
2. 使用支持 `.img.gz` 的工具，把完整镜像写入备用卡的整张设备。
   烧录前核对所选候选的 manifest 和 SHA256SUMS；无需在工作站保留解压后的大文件。
